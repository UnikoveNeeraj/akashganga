...
const SearchDropdown = (props) => {
  ...
  const handleOptionSelect = (value, index) => {
    if(value === "Others" && selectedValue.length < 3) {
  ...
  const onSearchChange = (e) => {
    ...
    if (e.target.value !== "") {
      let filterActivity = dropOptions.filter((ac) => {
        return ac.toLowerCase().includes(e.target.value.toLowerCase());
      });
      ...
      <div>
                ...
                <Menu key={i}>
                  <Menu.Item onClick={(e) => handleOptionSelect(exp, index)}>
                    <input
                      ...
                      name={name}
                      id={name}
                      checked={selectedValue?.includes(exp)}
        ...
        <Checkbox.Group>
          {dropOptions?.map((exp, i) => {
            if (!selectedValue?.includes(exp))
              return (
                <Menu key={i}>
                  <Menu.Item onClick={() => handleOptionSelect(exp, index)}>
                    <input
                      type="checkbox"
                      key={i}
                      value={exp}
  ...
  return (
    <>
      ...
      >
        ...
        // style={{ cursor: "pointer", pointerEvents:"none", opacity: "0.4" }}
        >
          <Collapse onChange={(e) => setActiveKey(activeKey === "1" ? null : "1")} activeKey={activeKey} expandIconPosition="right" className={selectedValue?.length !== 0 ? "value-fill" : "value"}>
            <Collapse.Panel  header={placeholder} key={"1"} >
              <p>{optionMenu}</p>
            </Collapse.Panel>
          </Collapse>
        </div>
